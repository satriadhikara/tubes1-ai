...
class SimulatedAnnealingState(State):
    def __init__(self, problem: Problem, jadwal =JadwalKuliah({}), randomizer: random.Random=random.Random(int(time.time()*1000))):
        super().__init__(problem, jadwal, randomizer)

        self.slot_assignment: dict[Slot, list[str]] = dict()
        self.empty_slots: list[Slot] = []
    ...
    def next(self, temperature: float) -> IterationResult:
        ...
        return IterationResult(delta_energy=e_neighbor-e_init, move_accepted=move_accepted)
    ...
    def _accept_move(self, delta_energy: float, temp: float) -> bool:
        ...
        return False
    # --- MOVE 1 ---
    # Requirement: idempotent or f(f(x)) = x
    ...
    # --- MOVE 2 ---
    # Requirement: f(a, b, c) undoes f(c, b, a)
    def _random_move_to_empty_slot(self) -> tuple[Slot, str, Slot]:
        ...
        slot_to = self.random.choice(self.empty_slots)
    ...
    def _move_into_slot(self, slot_from: Slot, kode: str, slot_to: Slot):
        ...
        if len(self.slot_assignment[slot_from])==0:
            ...
            self.empty_slots.append(slot_from)
        
        if slot_to in self.slot_assignment:
            self.slot_assignment[slot_to].append(kode)
        else:
            self.slot_assignment[slot_to] = [kode]
            self.empty_slots.remove(slot_to)


class SimulatedAnnealing(Solver):
    def __init__(self, input: Problem, initial_temp=DEFAULT_INITIAL_TEMP, decay=DEFAULT_DECAY_RATE):
